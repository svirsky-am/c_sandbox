...
pub trait AccountRepository: Send + Sync {
    ...
    async fn list_for_user(&self, user_id: Uuid) -> Result<Vec<Account>, DomainError>;
}
...
impl AccountRepository for InMemoryAccountRepository {
    async fn create(&self, account: Account) -> Result<(), DomainError> {
        ...
        if accounts.contains_key(&account.id) {
            return Err(DomainError::Validation("account already exists".into()));
        }
    ...
    async fn list_for_user(&self, user_id: Uuid) -> Result<Vec<Account>, DomainError> {
        ...
        Ok(accounts
            .values()
            .filter(|acc| acc.owner_id == user_id)
            .cloned()
            .collect())
    }
}
