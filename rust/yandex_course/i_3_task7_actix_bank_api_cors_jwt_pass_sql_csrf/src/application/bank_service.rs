    ...
    R: AccountRepository + 'static,
{
    ...
    pub async fn create_account(
        ...
        let account = Account::new(id, owner_id, initial_balance).map_err(BankError::from)?;
        self.repo.create(account.clone()).await.map_err(BankError::from)?;
        Ok(account)
    }
    ...
    #[instrument(skip(self))]
    pub async fn list_accounts(&self, owner: Uuid) -> Result<Vec<Account>, BankError> {
        self.repo.list_for_user(owner).await.map_err(BankError::from)
    }

    #[instrument(skip(self))]
    pub async fn deposit(&self, id: u32, amount: i64) -> Result<Account, BankError> {
    ...
    #[instrument(skip(self))]
    pub async fn withdraw(&self, id: u32, amount: i64) -> Result<Account, BankError> {
        let mut account = self.get_account(id).await?;
        let amount = Amount::new(amount).map_err(BankError::from)?;
        account
            .withdraw(amount)
    ...
    #[instrument(skip(self))]
    pub async fn transfer(&self, from: u32, to: u32, amount: i64) -> Result<(), BankError> {
